...
bool isFoldableUtil(tnode *pleft, tnode *pright) {
	if(!pleft && !pright) {
		return true;
	}

	if(pleft && pright) {
		return (isFoldableUtil(pleft->left, pright->left) && isFoldableUtil(pleft->right, pright->right));
	}
...
int main() {
	...
	if(isFoldable(root)) {
		...
		cout << "Trees is not foldable" << endl;
	}
