...
void generateSum(tnode *root, vector<int> &ivec, int slant) {
	if(!root) {
		return;
	}
	...
	if(slant >= ivec.size()) {
		...
		ivec[slant] += root->data;
	}

	generateSum(root->left, ivec, slant+1);
	generateSum(root->right, ivec, slant);
}

void printDiag(tnode *root) {
	if(!root) {
		return;
	}
	...
	generateSum(root->right, ivec, 0);
	generateSum(root->left, ivec , 1);
